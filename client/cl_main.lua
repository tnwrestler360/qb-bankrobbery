...
OnLaptopHackDone = function(success, bank)
    ...
    TriggerServerEvent('jpr-phonesystem:server:sendEmail', {
        Assunto = "Bank Security", -- Subject
        Conteudo = "The system should be down any minute. The vault door will open, grab the cash and GTFO!", -- Content
        Enviado = "sketchbot69@gmail.com", -- Submitted by
        Destinatario = QBCore.Functions.GetPlayerData().citizenid, -- Target
        Event = {type = "server", event = "jpr-phonesystem:example"}, -- Optional 
    })
end
...
CreateThread(function()
    ...
    lib.callback('qb-bankrobbery:server:GetConfig', false, function(result)
        Shared = result
    end)
